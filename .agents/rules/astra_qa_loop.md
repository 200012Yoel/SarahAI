...
3. 3D mode: improve visual hierarchy, viewport space, controls, manipulation feedback, scene/object selection, loading/error states, and overall usability. Preserve working behavior.
...
- A visual change alone is not sufficient when the feature has functional behavior. For voice, verify the recording/transcription state. For editor actions, verify editor/preview state. For 3D controls, verify the scene state actually changes.
...
## Regression pass
Test at minimum: cold launch, main chat, keyboard/input, plus menu, voice start/stop, navigation/sidebar, settings/about if present, 3D editor entry and core controls, code editor entry/edit/run-or-preview flow, back navigation, and return to chat. After every patch, rerun the affected flow. After the final patch, rerun the whole suite.
